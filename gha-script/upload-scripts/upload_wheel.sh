...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/octet-stream" -T $1 "https://s3.us-east.cloud-object-storage.appdomain.cloud/ose-s390x-artifacts/$PACKAGE_NAME/$VERSION/$1")
